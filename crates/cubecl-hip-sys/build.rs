...
fn get_system_hip_version(rocm_path: impl AsRef<Path>) -> std::io::Result<(u8, u8, u32)> {
    ...
    let mut version_lines = version_file.lines();

    // Checking for general file structure
    let [autogen_line, empty_line_1, ifndef_line, defguard_line, empty_line_2, major_line, minor_line, patch_line] =
        [
            version_lines.next(),
            version_lines.next(),
            version_lines.next(),
        ...
        .map(|line| line.expect("Invalid hip_version.h file structure: Not enough lines"));
    assert_eq!(
        autogen_line, "// Auto-generated by cmake",
        "Invalid hip_version.h file structure: First line is not an autogenerated comment"
    );
    assert_eq!(
        empty_line_1, "",
        "Invalid hip_version.h file structure: Second line is not empty"
    );
    assert_eq!(
        ifndef_line, "#ifndef HIP_VERSION_H",
        "Invalid hip_version.h file structure: Third line is not a header guard"
    );
    assert_eq!(
        defguard_line, "#define HIP_VERSION_H",
        "Invalid hip_version.h file structure: Fourth line is not a header guard"
    );
    assert_eq!(
        empty_line_2, "",
        "Invalid hip_version.h file structure: Fifth line is not empty"
    );
    let system_major = major_line
        .strip_prefix("#define HIP_VERSION_MAJOR ")
        .expect("Invalid hip_version.h file structure: Major version line structure mismatch")
        .parse::<u8>()
        .expect("Invalid hip_version.h file structure: Couldn't parse major version");
    let system_minor = minor_line
        .strip_prefix("#define HIP_VERSION_MINOR ")
        .expect("Invalid hip_version.h file structure: Minor version line structure mismatch")
        .parse::<u8>()
    ...
    let system_patch = patch_line
        .strip_prefix("#define HIP_VERSION_PATCH ")
        .expect("Invalid hip_version.h file structure: Patch version line structure mismatch")
        .parse::<u32>()
